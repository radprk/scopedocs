...
class OwnershipService:
    ...
    async def resolve_ownership(self, component_id: str) -> Dict[str, Any]:
        ...
        cursor = await db[COLLECTIONS['relationships']].find({
            'target_id': component_id,
            'relationship_type': RelationshipType.OWNS.value
        })
        ownership_rels = await cursor.to_list(100)
        ...
        if owner_ids:
            ...
            owners = await cursor.to_list(100)
        else:
            owners = []
    ...
    async def find_component_owners(self, repo: str, path: str = None) -> List[str]:
        ...
        cursor = await db[COLLECTIONS['components']].find(query)
        components = await cursor.to_list(100)
    ...
    async def get_ownership_summary(self) -> Dict[str, Any]:
        ...
        cursor = await db[COLLECTIONS['components']].find({})
